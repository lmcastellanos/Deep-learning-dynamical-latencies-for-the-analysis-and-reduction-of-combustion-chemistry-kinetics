...
for i in range(iterations):
    cantera_species=pd.read_csv(name_11+str(Condition_inputs.iloc[i,2])+name_12+str(Condition_inputs.iloc[i,1])+end)
    cantera_species=pd.DataFrame(cantera_species)
    
    cantera_sources=pd.read_csv(name_21+str(Condition_inputs.iloc[i,2])+name_22+str(Condition_inputs.iloc[i,1])+end)
...
reduced_size=1 #this is just for starting, we probably will change it later 

dataset_copy=dataset
dataset=dataset.sample(frac=1)

n_val = int(len(dataset)*.8)
training=dataset.iloc[0:n_val,:]
validation=dataset.iloc[n_val:,:]
...
training_fit_out=training.iloc[:,output_start:output_end].to_numpy()
training_fit_in=training.iloc[:,1:2+n_columns_mass_fraction].to_numpy()

validation_fit_out=validation.iloc[:,output_start:output_end].to_numpy()
...
def create_encoder(hp): 
    ...
    for j in range(num_layers):
        ...
        x=Dense(u,kernel_initializer=initializer, use_bias=True,bias_initializer=initializer_b)(x)
    ...
    x=Dense(units=reduced_size,activation='sigmoid',kernel_initializer=initializer, use_bias=True,bias_initializer=initializer_b)(x)
...
directory=f"H2_optimization_{reduced_size}"
...
models=tuner.search(training_fit_in, training_fit_out, epochs=100, validation_data=(validation_fit_in, validation_fit_out))
...
cantera_species=pd.read_csv(name_11+str(T)+name_12+str(Phi)+end)
cantera_species=pd.DataFrame(cantera_species)
    
cantera_sources=pd.read_csv(name_21+str(T)+name_22+str(Phi)+end)
cantera_sources=pd.DataFrame(cantera_sources)
